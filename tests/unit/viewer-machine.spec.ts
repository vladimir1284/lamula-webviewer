...
describe('viewerMachine — preferencias de display (D28)', () => {
  ...
  it('PREFS_LOADED asigna sin persistir (write-on-read prohibido)', () => {
    const { actor, persistPrefs } = boot({ initialRaster: meta(T0) })
    actor.start()
    actor.send({ type: 'PREFS_LOADED', prefs: { coverage: false, units: 'si', clock: 'local', animationFrames: 12, smooth: false, smoothRadius: 1, showPalette: true } })
  ...
  it('canario ensombrecido: ambos eventos surten efecto con las dos regiones paralelas activas', () => {
    ...
    const { actor } = boot({ initialRaster: meta(T0) })
    actor.start()
    actor.send({ type: 'PREFS_LOADED', prefs: { coverage: false, units: 'imperial', clock: 'utc', animationFrames: 12, smooth: false, smoothRadius: 1, showPalette: true } })
